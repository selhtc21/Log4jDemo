...
public class Member {
	...
	public boolean canCheck() {
		if (borrowedBooks.size() < 3) {
			logger.info(this.matricule + " can already check out :  checked out book = " + borrowedBooks.size() );
			return true;
		}
		logger.warn(this.matricule + " can not  check out :  checked out book = " + borrowedBooks.size() );
		return false;
	}
